...
import { useGetAttendanceRequestsQuery, useUpdateAttendanceRequestMutation, useGetStudentsQuery, useGetAttendanceQuery, useGetClassesQuery } from "@/services/api";
...
export default function TeacherAttendanceRequestsPage() {
    ...
    const isLoading = isLoadingRequests || isLoadingStudents || isLoadingAttendance || isLoadingClasses || !user;
    ...
    const teacherRequests = React.useMemo(() => {
        ...
        return requests.filter((req: any) => {
            const attendance = attendanceRecords.find((a: any) => a._id === req.attendanceId);
            return attendance && teacherClassIds.includes(attendance.classId);
        });
                    ...
                    <TableRow>
                        <TableCell colSpan={7}>
                             <EmptyState 
                                ...
                                description="There are currently no attendance requests to review from your students."
                            />
                        </TableCell>
                    </TableRow>
                </TableBody>
                    ...
                    return (
                         <TableRow key={req._id}>
                            ...
                            <TableCell className="text-right">
                                {req.status === 'pending' && (
                                    <>
                                        <Button size="icon" variant="ghost" className="text-green-600" onClick={() => handleUpdateRequest(req._id, 'approved')} disabled={isUpdating}><Check className="h-4 w-4"/></Button>
                                        <Button size="icon" variant="ghost" className="text-red-600" onClick={() => handleUpdateRequest(req._id, 'denied')} disabled={isUpdating}><X className="h-4 w-4" /></Button>
                                    </>
                                )}
                            </TableCell>
                        </TableRow>
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Review and approve or deny attendance change requests from your students.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>Change Requested</TableHead>
