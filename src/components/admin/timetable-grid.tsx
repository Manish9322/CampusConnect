...
export function TimetableGrid({ periods, onAddPeriod, onEditPeriod, onDeletePeriod }: TimetableGridProps) {
  ...
  const maxPeriodNumber = Math.max(8, sortedPeriods.length > 0 ? Math.max(...sortedPeriods.map(p => p.periodNumber)) : 0);

  // Create array of all period numbers
  const allPeriodNumbers = Array.from({ length: maxPeriodNumber }, (_, i) => i + 1);

  return (
    <div className="space-y-4">
      {allPeriodNumbers.map((periodNumber) => {
        const period = sortedPeriods.find(p => p.periodNumber === periodNumber);
        ...
        // Period with data
        return (
          <Card key={periodNumber} className="hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div className="flex-1 space-y-2">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{period.teacherName}</p>
                    </div>
        ...
        <Card className="border-dashed">
          <CardContent className="p-8 text-center">
            <p className="text-muted-foreground mb-4">
              No periods added yet. Click "Add Period" to create your first period.
            </p>
